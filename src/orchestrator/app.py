...
from agents.chat_completion.chat_completion_agent import get_chat_completion_agent, chat_completion_agent_run
...
async def update_image_review_and_textvector_v2(preprocess_result: Dict[str, Any]):
    ...
    AzureOpenAISettings(
        ...
        api_key=os.getenv("AZREU_TEXT_EMBEDDING_APIKEY"),
    ...
    chat_completion_agent = await get_chat_completion_agent()
    ...
    logging.info(f"chat history agent in thread : {thread}")
...
async def ingest_video_v2(file: UploadFile = File(...), container_name: str = Form("videos"), frames_container: str = Form("frames")):
    ...
    try:
        ...
        preprocess_result = await update_image_review_and_textvector_v2(preprocess_result)
